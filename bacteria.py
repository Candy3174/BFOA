...
class bacteria():
    ...
    def mutacion(self, poblacion, tasa_mutacion):
        """
        Aplica mutaciones controladas a las secuencias de las bacterias.
        :param poblacion: Lista de bacterias (población actual).
        :param tasa_mutacion: Probabilidad de mutación para cada carácter.
        """
        for i in range(len(poblacion)):
            bacterTmp = list(poblacion[i])
            for j in range(len(bacterTmp)):  # Recorre las secuencias de la bacteria
                for k in range(len(bacterTmp[j])):  # Recorre los caracteres de la secuencia
                    if random.random() < tasa_mutacion:
                        # Reemplaza el carácter actual con un carácter aleatorio
                        bacterTmp[j][k] = random.choice("ACGT-")  # Asumiendo secuencias de ADN
            poblacion[i] = tuple(bacterTmp)
